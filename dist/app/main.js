...
function ceilToNextMinutes(date, stepMinutes) {
    const ms = date.getTime();
    const stepMs = stepMinutes * 60_000;
    const ceiled = Math.ceil(ms / stepMs) * stepMs;
    return new Date(ceiled);
}
function buildSuggestedSlots(now = new Date()) {
...
async function askDateTime(rl, prompt) {
    while (true) {
...
function ensureRoom(control, type) {
...
function getRoomAddons(room) {
    const withAddons = room;
    ...
    const anyRoom = room;
    const labels = [];
    if (anyRoom?.projector)
        labels.push('Projetor');
    if (anyRoom?.cleaning)
        labels.push('Serviço de limpeza');
    if (anyRoom?.coffee)
        labels.push('Área de café');
    return labels;
}
function printRoomAddons(room) {
    const addons = getRoomAddons(room);
    console.log(`Equipamentos adicionais: ${addons.length > 0 ? addons.join(', ') : 'Nenhum'}`);
}
async function maybeDecorateRoom(rl, control, roomId) {
    ...
    if (addonChoice === '1')
        decorated = new ProjectorRoom(decorated);
    if (addonChoice === '2')
        decorated = new CleaningServiceRoom(decorated);
    if (addonChoice === '3')
        decorated = new CoffeeAreaRoom(decorated);
    control.rooms[roomIndex] = decorated;
    console.log('Adicional aplicado com sucesso.');
    printRoomAddons(decorated);
}
async function handleRegisterUser(rl, control) {
    printUserSubMenu();
...
async function handleScheduleRoom(rl, control) {
...
async function handleConsultReservations(rl, proxy) {
    ...
    reservations.forEach((reservation) => {
        ...
        console.log(`Reserva ID=${reservation.id}: ${userName} (${userRole}) reservou Sala ${roomId} (${roomType}) | ` +
...
export async function main() {
    ...
    try {
        while (true) {
            printMainMenu();
            const choice = await askChoice(rl, 'Escolha uma opção (1-4): ', ['1', '2', '3', '4']);
            if (choice === '3') {
